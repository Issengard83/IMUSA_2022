...
inf_shp <- st_read("raw/shp/Registro Nacional de Barrios Populares - Santa Fe.shp", 
                   stringsAsFactors = T) %>% 
...
### IMUSA markers
imu_loc <- st_read("clean/shp/IMUSA_SF.geojson")


### Socioeconomic indicators by census tract
var_ct <- st_read("clean/shp/VAR_CT_SF.geojson") %>% 
...
imu_shp <- import("clean/data_IMUSA_clean.xlsx") %>% 
  ...
  select(id_muestra, origen_muestra, lon, lat, animal, MAT_res) %>% 
  ...
  ## Drop unused columns
  select(id_muestra, origen_muestra, animal, MAT_res, redcode, adm_dis,
         pc_incid_h, p_casab_ho, ends_with("_pct"))
...
imu_p_ct <- imu_shp %>% 
  ...
  group_by(across(redcode:h_residuos_pct)) %>% 
  
  summarise(
    n_sampled = n(),
    
    n_pos = if_else(MAT_res == "POS", 1, 0) %>% 
      sum(na.rm = T)
  ) %>% 
  ...
  ### Add prevalence
  mutate(seroprev = round(n_pos*100/n_sampled, 1)) %>% 
...
fig1 <- base_map +
  ...
  tm_bubbles(
    ...
    jitter = .1,
    # title.col = "Animal",
    title.size ="Number of sampled dogs and cats",
    legend.col.show = F,
    legend.size.show = T,
...
### Save map
# tmap_save(fig1, "FIGS/fig1.jpg", dpi = 300, width = 1063, height = 1600, units = "px")


### Clean working environment
rm(imu_loc, imu_shp, imu_shp_2, base_osm, fig1)
...
imu_ct %>% 
  select(n_pos, adm_dis, pc_incid_h, pc_hog_pct, h_nbi_pct, p_casab_ho,
         h_hacinami_pct, h_cloaca_pct, h_hoyo_pct, h_agua_viv_pct, 
         h_paviment_pct, h_residuos_pct) %>% 
  tbl_uvregression(y = n_pos, 
...
## Univariate models
fit1 = glmmTMB(n_pos ~ pc_incid_h + (1|adm_dis), family = poisson,
               data = imu_ct)

fit2 = glmmTMB(n_pos ~ h_nbi_pct + (1|adm_dis), family = poisson,
               data = imu_ct)


fit3 = glmmTMB(n_pos ~ p_casab_ho + (1|adm_dis), family = poisson,
               data = imu_ct)

fit4 = glmmTMB(n_pos ~ h_residuos_pct + (1|adm_dis), family = poisson,
               data = imu_ct)

## Multivariate model
fit5 = glmmTMB(n_pos ~ p_casab_ho + h_residuos_pct + (1|adm_dis), family = poisson,
               data = imu_ct)
...
fig2 <- base_map +
  ...
  tm_bubbles(size = "seroprev",
             col = "#FFC300",
             alpha = .6,
             title.size = "Seroprevalence (%)",
             legend.size.show = T) +
...
# ### Save map
# tmap_save(fig2, "FIGS/fig2.jpg", width = 2244, height = 1500, units = "px", dpi = 300)
...
# ### Save map
# tmap_save(fig3, "FIGS/fig3.jpg", width = 2244, height = 2000, units = "px", dpi = 300)
...
imu_poly_ct <- var_ct %>% 
  ...
  inner_join(imu_p_ct %>% st_drop_geometry() %>% 
               select(redcode, n_sampled, n_pos)) %>% 
  
  # Replace missing values with 0
  mutate_at(c("n_sampled","n_pos"), .funs = ~ replace_na(.x, 0))
...
gmoran <- moran.test(imu_poly_ct$n_pos, listw = nbw, alternative = "greater")

gmoran$p.value

# Plot results
moran.plot(imu_poly_ct$n_pos, nbw)


# Local Moran's I (census tract) ------------------------------------------
lmoran <- localmoran(imu_poly_ct$n_pos, listw = nbw, alternative = "two.sided")

### Identify clusters
mp <- imu_poly_ct$n_pos %>% 
  scale() %>% 
  as.vector() %>% 
  moran.plot(., nbw) 
...
fig4 <- base_map +
  ...
  tm_fill(col = c("n_pos","Ii","Zii_cat","quadrant"),
          # style = "pretty",
          alpha = .9,
          # palette = "viridis",
...
### Save map
tmap_save(fig4, "FIGS/fig4.jpg", width = 2244, height = 1500, units = "px", dpi = 300)
